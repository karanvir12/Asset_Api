        ...
        if (queryAt.parachainStaking && queryAt.parachainStaking.selectedCandidates && queryAt.session && queryAt.session.nextKeys && queryAt.session.nextKeys.multi) {
            return (0, rxjs_1.combineLatest)([
                ...
                queryAt.parachainStaking.selectedCandidates().pipe((0, rxjs_1.mergeMap)((selectedCandidates) => (0, rxjs_1.combineLatest)([
                    ...
                    queryAt.session.nextKeys.multi(selectedCandidates).pipe((0, rxjs_1.map)((nextKeys) => nextKeys.findIndex((option) => option.unwrapOrDefault().nimbus.toHex() === loggedAuthor.toHex())))
                ])), (0, rxjs_1.map)(([selectedCandidates, index]) => selectedCandidates[index]))
            ]);
        }
    }
